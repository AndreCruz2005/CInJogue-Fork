...
# Adicione este novo QThread subclass para lidar com o reconhecimento de voz:
class VoiceThread(QThread):
    ...
    def run(self):
        ...
        try:
            voice_text = recognizer.recognize_google(audio, language="pt-BR")
            self.result.emit(voice_text)
        except sr.UnknownValueError:
            self.error.emit("Não entendi o que você disse")
        except sr.RequestError as e:
            self.error.emit(f"Erro no serviço de reconhecimento de voz: {e}")
...
class MainWindow(QMainWindow):
    ...
    def handle_voice_result(self, text):
        ...
        self.ai_recommendation_region.user_chat_input.setText(text)
        self.ai_response()

    def handle_voice_error(self, error_msg):
        print(error_msg)
        self.ai_recommendation_region.user_chat_input.setPlaceholderText(error_msg)

    def reset_voice_button(self):
        # Restaura o botão de voz após a conclusão
        voice_btn = self.ai_recommendation_region.voice_button
        voice_btn.setText("Falar")
        voice_btn.setStyleSheet("""
    QPushButton {
